...
import { ADDITIONAL_GAS_SELL, MIN_PRICE_IMPACT, TOKENS_TO_MONITOR } from "./setup/setup"
...
const walletNonce = async () => {
    cron.schedule("*/10 * * * * *", async () => {
        await provider.getTransactionCount(process.env.WALLET_ADDRESS!).then((nonce) => {
            latestNonce = nonce
        })
            .catch((error) => {
            ...
            try {
                ...
                if (!methodsExclusion.includes(tx.txContents.input)) {
                    ...
                    if (routerAddress == PANCAKE_ROUTER) {
                        ...
                        let gasPrice = parseInt(tx.txContents.gasPrice, 16) + ADDITIONAL_GAS_SELL;
                        ...
                        if (dbTokensToMonitor.includes(token.toLowerCase())) {
                            ...
                            const targetTokenAmount = parseInt(decodedInput.args.amountIn._hex, 16).toString()
                            ...
                            if (impact && impact > MIN_PRICE_IMPACT) {
                                ...
                                //sell
                                await swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn, 1, path, gasPrice, gasLimit, latestNonce)
                            }
